...
# Create instance of Bert tokenizer
...
for featurefile in glob.glob('**/features.json.lz4', recursive=True, root_dir=dataset):
    try:
        ...
        str_list = sorted(j["COMMON"]["STRINGS"], key=len, reverse=True)
        ...
        for size in sizes:
            output_file = f"{dataset}/{os.path.dirname(featurefile)}/strings_{size}.feature"
            ...
            with open(output_file, 'w') as outfile:
                json.dump({"STRINGS" : input_list}, outfile)
